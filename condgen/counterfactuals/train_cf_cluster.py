...
def main(model_cls, data_cls, args):
    ...
    num_classes_model_og = copy.copy(args.num_classes_model) #store the num_classes_model for later
    ...
    early_stopping_cb = EarlyStopping(monitor="val_loss", patience=args.early_stopping1)

    trainer = pl.Trainer(gpus = args.gpus, logger = logger, callbacks = [checkpoint_cb, early_stopping_cb], max_epochs = args.max_epochs1)
    ...
    imgs_pred = torch.cat([pred["imgs_pred"] for pred in preds]).detach().cpu()[:,0]
    ...
    noise_pred = torch.cat([pred["noise_prediction"] for pred in preds]).detach().cpu()
    ...
    early_stopping_cb2 = EarlyStopping(monitor="val_loss", patience=args.early_stopping2)

    trainer = pl.Trainer(gpus = args.gpus, logger = logger, callbacks = [checkpoint_cb2, early_stopping_cb2], max_epochs = args.max_epochs2)
    trainer.fit(model, datamodule = dataset)
...
if __name__=="__main__":
    ...
    parser.add_argument('--repeat', default=1, type=int,help = "dummy argument for running multiple times")
